...
class BinaryBlob:
    ...
    def __init__(self, start_address=None, end_address=None, label=None, debug=None, line_number=None):
    ...
    for line in redsrc:
        ...
        if line[0:7] == "SECTION":
            ...
            if (":" in line) and (not ";XXX:" in line) and (not " ; XXX:" in line):
                ...
                current_start_address = get_address_from_line_comment(line, bank=current_bank)

                if (current_label is not None) and (current_start_address is not None) and (latest_start_address is not None) \
        ...
        if current_bank == bank_stop:
            if debug:
                print(f"scan_red_asm: stopping because current_bank >= {str(bank_stop)} (bank_stop)")
            break
