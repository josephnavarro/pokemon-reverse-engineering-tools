...
def get_base_command_id(command_id: int, channel: int = 1, is_sfx: bool = False) -> int:
...
def get_bank(address: int) -> int:
    return address // 0x4000



def get_pointer(address: int, bank=None) -> int:
    """
    ...
    """
    if bank is None:
        bank: int = get_bank(address)
    return (rom[address + 1] * 0x100 + rom[address]) % 0x4000 + bank * 0x4000


def is_infinite_loop(address: int) -> bool:
    """
    ...
    """
    return ((rom[address] == 0xfd) and (rom[address + 1] == 0)) or (rom[address] == 0xfc)


def make_blob(start, output, end=None, label=None) -> dict:
    return {
        "start": start,
        "output": output,
...
def dump_channel(start_address: int, sound_name: str, channel: int, prefix: str = "", is_sfx: bool = True, address=None):
    """
    ...
    if address is None:
        blobs.append(make_blob(start_address, f"{prefix}{sound_name}_Ch{channel}:\n"))
        address: int = start_address
        if (sound_name == "MagnetTrain") and (channel == 4):
            unseen_branch: int = 0xef711
            unseen_label: str = "; unused\n{0:s}{1:s}_branch_{2:x}".format(prefix, sound_name, unseen_branch)
            labels.append(
                {
                    "address": unseen_branch,
                    "label": unseen_label,
                }
            )
    ...
    while True:
        ...
        command_address: int = address
        command_id: int = rom[command_address]
...
def dump_all_sounds(header_pointer, sound_names, prefix="", is_sfx=True) -> list:
    blobs: list = []
...
def fill_gap(start: int, end: int) -> str:
    output: str = ""
...
def sort_and_filter(blobs: list, extra_labels: list = []) -> list:
    ...
    for blob, next in zip(blobs, blobs[1:]+[None]):
        ...
        elif blob["label"] is not None:
            label_pos: int = blob["output"].rfind(" ") + 1
            label_address: int = int(blob["output"][label_pos:], 16)
...
def write_all_sounds_to_file(path: str, file, blobs):
    try:
        print(f"Writing {path + file}...")

        os.makedirs(path, exist_ok=True)

        with open(path + file, "w") as sound_file:
            for blob in blobs[:-1]:
                sound_file.write(blob["output"])

    except IOError as ex:
        print(f"Error writing {path + file}")
        print(ex)
...
def export_all_sounds(path, header_pointer, sound_names, prefix="", is_sfx=True):
    ...
    for blobs, sound_name in zip(sounds, sound_names):
        ...
        write_all_sounds_to_file(path, f"{sound_name.lower()}.asm", blobs)
